...
settingsBtn.addEventListener('click', () => {
  chrome.runtime.sendMessage({ type: 'get_settings' }, (resp) => {
    ...
    downloadFolderDisplay.textContent = (resp && resp.downloadSubfolder) || 'ASAP Transcripts';
...
// Browse button: opens a real OS folder picker via the File System Access API.
// We use the chosen folder's name as the subfolder within Downloads.
btnBrowseFolder.addEventListener('click', async () => {
  try {
    const handle = await window.showDirectoryPicker({ mode: 'readwrite' });
    const folderName = handle.name;
    chrome.runtime.sendMessage({ type: 'save_download_folder', folder: folderName }, resp => {
      if (resp && resp.ok) {
        downloadFolderDisplay.textContent = folderName;
        settingsStatus.style.color = 'var(--green)';
        settingsStatus.textContent = `Folder set to "${folderName}".`;
      }
    });
  } catch (e) {
    if (e.name !== 'AbortError') {
      settingsStatus.style.color = 'var(--red)';
      settingsStatus.textContent = 'Could not open folder picker.';
    }
  }
});
